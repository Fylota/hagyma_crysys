...
int64_t ParseUtils::parse8ByteNumber(std::vector<uint8_t> &bytes, uint64_t startIndex, Endianess endianess) {
    ...
    if (endianess == Endianess::LITTLE_ENDIAN_MODE) {
        int i = 0;
    ...
    } else if (endianess == Endianess::BIG_ENDIAN_MODE) {
        int i = 0;
        while (i < 8) {
            result = (result << 8) | bytes[startIndex];
            startIndex++;
...
int16_t ParseUtils::parse2ByteNumber(std::vector<uint8_t> &bytes, uint64_t startIndex, Endianess endianess) {
    int16_t result = 0;
    const int16_t shiftNumber = 8;

    if (endianess == Endianess::LITTLE_ENDIAN_MODE) {
        int i = 0;
        startIndex = startIndex + 1;
        while (i < 2) {
            result = (int16_t)((result << shiftNumber) | bytes[startIndex]);
            startIndex--;
            i++;
        }
    } else if (endianess == Endianess::BIG_ENDIAN_MODE) {
